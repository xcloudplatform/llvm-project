; RUN: llc -march=sbf -filetype=obj -o - %s | llvm-objdump -d - | FileCheck --check-prefix=CHECK-DEC %s
; RUN: llc -march=sbf -filetype=obj -o - %s | llvm-objdump -d --print-imm-hex - | FileCheck --check-prefix=CHECK-HEX %s
...
; CHECK-DEC: 18 03 00 00 cd ab cd ab 00 00 00 00 cd ab cd ab         lddw r3, -6067004223159161907
; CHECK-DEC: 5d 31 07 00 00 00 00 00         jne r1, r3, +7
...
; <label>:4:                                      ; preds = %2
  ...
  %6 = shl i32 %5, 1
; CHECK-DEC: 67 01 00 00 01 00 00 00         lsh64 r1, 1
...
; CHECK-DEC: 18 01 00 00 cd ab cd ab 00 00 00 00 cd ab 00 00         lddw r1, 188899839028173
...
; CHECK-DEC: 63 12 00 00 00 00 00 00         stxw [r2 + 0], r1

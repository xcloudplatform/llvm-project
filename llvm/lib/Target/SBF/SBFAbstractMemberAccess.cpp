//===------ SBFAbstractMemberAccess.cpp - Abstracting Member Accesses -----===//
//
...
#define DEBUG_TYPE "sbf-abstract-member-access"

namespace llvm {
constexpr StringRef SBFCoreSharedInfo::AmaAttr;
uint32_t SBFCoreSharedInfo::SeqNum;
...
namespace {
class SBFAbstractMemberAccess final {
public:
  SBFAbstractMemberAccess(SBFTargetMachine *TM) : TM(TM) {}
...
private:
  enum : uint32_t {
    SBFPreserveArrayAI = 1,
    SBFPreserveUnionAI = 2,
    SBFPreserveStructAI = 3,
    SBFPreserveFieldInfoAI = 4,
  };
...
void SBFAbstractMemberAccess::CheckCompositeType(DIDerivedType *ParentTy,
                                                 DICompositeType *CTy) {
...
void SBFAbstractMemberAccess::CheckAnonRecordType(DIDerivedType *ParentTy,
                                                  DIType *Ty) {
  if (!Ty)
    return;
...
bool SBFAbstractMemberAccess::IsPreserveDIAccessIndexCall(const CallInst *Call,
                                                          CallInfo &CInfo) {
  if (!Call)
    return false;
  ...
  if (GV->getName().startswith("llvm.preserve.struct.access.index")) {
    CInfo.Kind = SBFPreserveStructAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
  ...
  if (GV->getName().startswith("llvm.bpf.preserve.field.info")) {
    CInfo.Kind = SBFPreserveFieldInfoAI;
    ...
    if (InfoKind >= SBFCoreSharedInfo::MAX_FIELD_RELOC_KIND)
  ...
  if (GV->getName().startswith("llvm.bpf.preserve.type.info")) {
    ...
    if (Flag >= SBFCoreSharedInfo::MAX_PRESERVE_TYPE_INFO_FLAG)
  ...
  if (GV->getName().startswith("llvm.bpf.preserve.enum.value")) {
    ...
    if (Flag >= SBFCoreSharedInfo::MAX_PRESERVE_ENUM_VALUE_FLAG)
      report_fatal_error("Incorrect flag for llvm.bpf.preserve.enum.value intrinsic");
    if (Flag == SBFCoreSharedInfo::PRESERVE_ENUM_VALUE_EXISTENCE)
      CInfo.AccessIndex = SBFCoreSharedInfo::ENUM_VALUE_EXISTENCE;
    else
      CInfo.AccessIndex = SBFCoreSharedInfo::ENUM_VALUE;
    return true;
  }
...
bool SBFAbstractMemberAccess::removePreserveAccessIndexIntrinsic(Function &F) {
  ...
  for (auto &BB : F)
    for (auto &I : BB) {
      ...
      Found = true;
      if (CInfo.Kind == SBFPreserveArrayAI)
        PreserveArrayIndexCalls.push_back(Call);
      else if (CInfo.Kind == SBFPreserveUnionAI)
...
bool SBFAbstractMemberAccess::IsValidAIChain(const MDNode *ParentType,
                                             uint32_t ParentAI,
...
void SBFAbstractMemberAccess::traceAICall(CallInst *Call,
                                          CallInfo &ParentInfo) {
...
void SBFAbstractMemberAccess::traceBitCast(BitCastInst *BitCast,
                                           CallInst *Parent,
                                           CallInfo &ParentInfo) {
...
void SBFAbstractMemberAccess::traceGEP(GetElementPtrInst *GEP, CallInst *Parent,
                                       CallInfo &ParentInfo) {
...
void SBFAbstractMemberAccess::collectAICallChains(Function &F) {
  AIChain.clear();
  BaseAICalls.clear();
...
uint64_t SBFAbstractMemberAccess::getConstant(const Value *IndexValue) {
...
void SBFAbstractMemberAccess::GetStorageBitRange(DIDerivedType *MemberTy,
...
uint32_t SBFAbstractMemberAccess::GetFieldInfo(uint32_t InfoKind,
                                               DICompositeType *CTy,
                                               uint32_t AccessIndex,
                                               uint32_t PatchImm,
                                               MaybeAlign RecordAlignment) {
  ...
  if (InfoKind == SBFCoreSharedInfo::FIELD_BYTE_OFFSET) {
  ...
  if (InfoKind == SBFCoreSharedInfo::FIELD_BYTE_SIZE) {
    if (Tag == dwarf::DW_TAG_array_type) {
      auto *EltTy = stripQualifiers(CTy->getBaseType());
  ...
  if (InfoKind == SBFCoreSharedInfo::FIELD_SIGNEDNESS) {
    const DIType *BaseTy;
    if (Tag == dwarf::DW_TAG_array_type) {
  ...
  if (InfoKind == SBFCoreSharedInfo::FIELD_LSHIFT_U64) {
  ...
  if (InfoKind == SBFCoreSharedInfo::FIELD_RSHIFT_U64) {
    DIDerivedType *MemberTy = nullptr;
    bool IsBitField = false;
    uint32_t SizeInBits;
    if (Tag == dwarf::DW_TAG_array_type) {
...
bool SBFAbstractMemberAccess::HasPreserveFieldInfoCall(CallInfoStack &CallStack) {
  // This is called in error return path, no need to maintain CallStack.
  while (CallStack.size()) {
    auto StackElem = CallStack.top();
    if (StackElem.second.Kind == SBFPreserveFieldInfoAI)
      return true;
    CallStack.pop();
  }
  return false;
...
Value *SBFAbstractMemberAccess::computeBaseAndAccessKey(CallInst *Call,
                                                        CallInfo &CInfo,
  ...
  uint32_t InfoKind = SBFCoreSharedInfo::FIELD_BYTE_OFFSET;
  while (CallStack.size()) {
    auto StackElem = CallStack.top();
    ...
    if (CInfo.Kind == SBFPreserveUnionAI ||
        CInfo.Kind == SBFPreserveStructAI) {
    ...
    assert(CInfo.Kind == SBFPreserveArrayAI);
  ...
  while (CallStack.size()) {
    ...
    if (CInfo.Kind == SBFPreserveFieldInfoAI) {
    ...
    // If the next Call (the top of the stack) is a SBFPreserveFieldInfoAI,
    // the action will be extracting field info.
    if (CallStack.size()) {
      auto StackElem2 = CallStack.top();
      CallInfo CInfo2 = StackElem2.second;
      if (CInfo2.Kind == SBFPreserveFieldInfoAI) {
...
MDNode *SBFAbstractMemberAccess::computeAccessKey(CallInst *Call,
                                                  CallInfo &CInfo,
                                                  std::string &AccessKey,
                                                  bool &IsInt32Ret) {
  ...
  if (CInfo.AccessIndex == SBFCoreSharedInfo::TYPE_EXISTENCE ||
    ...
    if (CInfo.AccessIndex == SBFCoreSharedInfo::ENUM_VALUE) {
...
bool SBFAbstractMemberAccess::transformGEPChain(CallInst *Call,
                                                CallInfo &CInfo) {
  std::string AccessKey;
  MDNode *TypeMeta;
  Value *Base = nullptr;
  ...
  IsInt32Ret = CInfo.Kind == SBFPreserveFieldInfoAI;
  if (CInfo.Kind == SBFPreserveFieldInfoAI && CInfo.Metadata) {
  ...
  if (GEPGlobals.find(AccessKey) == GEPGlobals.end()) {
    ...
    GV = new GlobalVariable(*M, VarType, false, GlobalVariable::ExternalLinkage,
                            nullptr, AccessKey);
    GV->addAttribute(SBFCoreSharedInfo::AmaAttr);
  ...
  Instruction *PassThroughInst =
      SBFCoreSharedInfo::insertPassThrough(M, BB, BCInst2, Call);
  Call->replaceAllUsesWith(PassThroughInst);
  Call->eraseFromParent();
...
bool SBFAbstractMemberAccess::doTransformation(Function &F) {
...
PreservedAnalyses
SBFAbstractMemberAccessPass::run(Function &F, FunctionAnalysisManager &AM) {
  return SBFAbstractMemberAccess(TM).run(F) ? PreservedAnalyses::none()

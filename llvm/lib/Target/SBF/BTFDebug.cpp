...
void BTFDebug::processGlobalValue(const MachineOperand &MO) {
  ...
  if (MO.isGlobal()) {
    ...
    if (!GVar->hasAttribute(SBFCoreSharedInfo::AmaAttr) &&
        !GVar->hasAttribute(SBFCoreSharedInfo::TypeIdAttr))
      return;
    ...
    generatePatchImmReloc(ORSym, RootId, GVar,
                          GVar->hasAttribute(SBFCoreSharedInfo::AmaAttr));
  }
}
  ...
  if (MI->getOpcode() == SBF::LD_imm64) {
    ...
    if (MO.isGlobal()) {
      ...
      if (GVar) {
        ...
        if (Reloc == SBFCoreSharedInfo::ENUM_VALUE_EXISTENCE ||
            Reloc == SBFCoreSharedInfo::ENUM_VALUE ||
            Reloc == SBFCoreSharedInfo::BTF_TYPE_ID_LOCAL ||
            Reloc == SBFCoreSharedInfo::BTF_TYPE_ID_REMOTE)
          OutMI.setOpcode(SBF::LD_imm64);
        else
          OutMI.setOpcode(SBF::MOV_ri);
    ...
    if (MO.isGlobal()) {
      const GlobalValue *GVal = MO.getGlobal();
      auto *GVar = dyn_cast<GlobalVariable>(GVal);
      if (GVar && GVar->hasAttribute(SBFCoreSharedInfo::AmaAttr)) {

//===------------ SBFIRPeephole.cpp - IR Peephole Transformation ----------===//
//
...
#define DEBUG_TYPE "sbf-ir-peephole"

using namespace llvm;

namespace {

static bool SBFIRPeepholeImpl(Function &F) {
  LLVM_DEBUG(dbgs() << "******** SBF IR Peephole ********\n");
  ...
  for (auto &BB : F) {
    for (auto &I : BB) {
      ...
      // since SBF does not support dynamic allocation yet.
      if (ToErase) {

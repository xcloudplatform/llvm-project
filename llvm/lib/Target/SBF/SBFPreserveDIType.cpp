//===----------- SBFPreserveDIType.cpp - Preserve DebugInfo Types ---------===//
//
...
namespace llvm {
constexpr StringRef SBFCoreSharedInfo::TypeIdAttr;
} // namespace llvm
...
static bool SBFPreserveDITypeImpl(Function &F) {
  ...
  for (auto Call : PreserveDITypeCalls) {
    ...
    if (FlagValue >= SBFCoreSharedInfo::MAX_BTF_TYPE_ID_FLAG)
    ...
    uint32_t Reloc;
    if (FlagValue == SBFCoreSharedInfo::BTF_TYPE_ID_LOCAL_RELOC) {
      Reloc = SBFCoreSharedInfo::BTF_TYPE_ID_LOCAL;
    } else {
      Reloc = SBFCoreSharedInfo::BTF_TYPE_ID_REMOTE;
    ...
    GV->addAttribute(SBFCoreSharedInfo::TypeIdAttr);
...
PreservedAnalyses SBFPreserveDITypePass::run(Function &F,
  ...
  return SBFPreserveDITypeImpl(F) ? PreservedAnalyses::none()

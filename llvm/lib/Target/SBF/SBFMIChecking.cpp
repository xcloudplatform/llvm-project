...
// used.
//
// NOTE: SBF backend hasn't enabled sub-register liveness track, so when the
...
// on SBF backend, then allDefsAreDead could work on GPR32 Def. This requires
// implementing TargetSubtargetInfo::enableSubRegLiveness on SBF.
//
...
// each make sense.
//
// For SBF, each 64-bit register could only have one 32-bit sub-register. This
...
//
// Hence, for SBF, we enhance MachineInstr::allDefsAreDead. Given the solo
...
static bool hasLiveDefs(const MachineInstr &MI, const TargetRegisterInfo *TRI) {
  const MCRegisterClass *GPR64RegClass =
    &SBFMCRegisterClasses[SBF::GPRRegClassID];
...
bool SBFMIPreEmitChecking::processAtomicInsts() {
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      if (MI.getOpcode() != SBF::XADDW &&
          MI.getOpcode() != SBF::XADDD &&
          MI.getOpcode() != SBF::XADDW32)
        continue;
  ...
  bool Changed = false;
  const SBFInstrInfo *TII = MF->getSubtarget<SBFSubtarget>().getInstrInfo();
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      if (ToErase) {
      ...
      if (MI.getOpcode() != SBF::XFADDW32 && MI.getOpcode() != SBF::XFADDD &&
          MI.getOpcode() != SBF::XFANDW32 && MI.getOpcode() != SBF::XFANDD &&
          MI.getOpcode() != SBF::XFXORW32 && MI.getOpcode() != SBF::XFXORD &&
          MI.getOpcode() != SBF::XFORW32 && MI.getOpcode() != SBF::XFORD)
        continue;
      ...
      switch (MI.getOpcode()) {
      case SBF::XFADDW32:
        newOpcode = SBF::XADDW32;
        break;
      case SBF::XFADDD:
        newOpcode = SBF::XADDD;
        break;
      case SBF::XFANDW32:
        newOpcode = SBF::XANDW32;
        break;
      case SBF::XFANDD:
        newOpcode = SBF::XANDD;
        break;
      case SBF::XFXORW32:
        newOpcode = SBF::XXORW32;
        break;
      case SBF::XFXORD:
        newOpcode = SBF::XXORD;
        break;
      case SBF::XFORW32:
        newOpcode = SBF::XORW32;
        break;
      case SBF::XFORD:
        newOpcode = SBF::XORD;
        break;
